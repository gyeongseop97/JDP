...
function showResult(id){if(mode==='tutorial')return;if(innerWidth<=760)requestAnimationFrame(()=>$(id)?.scrollIntoView({block:'start',behavior:motion()}))}
...
function renderDialogue(o){
  const history=o.history||[];const latest=history.filter(h=>h.speaker==='customer').at(-1);
  ...
  $('patience-label').textContent=o.patience<=1?'곧 떠날 것 같아요':o.mood<-10?'기분이 상했어요':o.mood>10?'호의적인 반응':'이야기를 듣고 있어요';
  $('dialogue-log').innerHTML=history.map(h=>'<div class="dialogue-line '+esc(h.speaker)+'"><span>'+({player:'나',customer:PAWN_CHARACTERS[o.character].name,note:'관찰'}[h.speaker]||'기록')+'</span><p>'+esc(h.text)+'</p></div>').join('')||'<p class="quiet-note">첫 질문으로 대화를 시작해 보세요.</p>';
  const log=$('dialogue-log');log.scrollTop=log.scrollHeight;
  const options=E.dialogueOptions(s)||[];
  $('dialogue-options').innerHTML=options.length?options.map(x=>'<button class="dialogue-choice '+(x.tone==='risk'||x.tone==='hostile'?'risky':'')+'" data-talk="'+esc(x.id)+'"><strong>'+esc(x.label)+'</strong>'+(x.hint?'<small>'+esc(x.hint)+'</small>':'')+'</button>').join(''):'<p class="quiet-note">충분히 이야기를 나눴어요. 가격을 제안하거나 거래를 마무리하세요.</p>';
...
function renderOffer(o){
  renderBuyerStock(o);renderDialogue(o);
...
function render(){
  ...
  $('coins').innerHTML=(s.coins<100000?fmt(s.coins):new Intl.NumberFormat('ko-KR',{notation:'compact',maximumFractionDigits:1}).format(s.coins))+'<span>G</span>';$('coins').setAttribute('aria-label',fmt(s.coins)+' 골드');$('coins').title=fmt(s.coins)+' G';
  ...
  renderRelationship(o);$('scene-grade').textContent=known?(o.genuine?'진품':'모조품'):'손님의 주장 · 아직 미확인';$('scene-item').dataset.item=it.id;$('scene-item').dataset.grade=known?o.grade:1;
...
function startGame(options={}){if(options.fresh)s=E.prepare(E.create());mode='play';tutorialBackup=null;offerKey='';$('detail-dialog').close();signalMode();switchView('trade');save()}
function returnToTitle(){if(mode==='tutorial'){endTutorial();return}save();$('detail-dialog').close();mode='menu';signalMode();window.scrollTo(0,0)}
function beginTutorial(seed){if(mode!=='tutorial')tutorialBackup=JSON.parse(JSON.stringify(s));mode='tutorial';s=E.prepare(E.create(seed));offerKey='';$('detail-dialog').close();signalMode();switchView('trade')}
...
function endTutorial(){if(tutorialBackup)s=tutorialBackup;tutorialBackup=null;mode='menu';offerKey='';$('detail-dialog').close();signalMode();render();window.scrollTo(0,0)}
