...
(() => {
  ...
  function sellerScene(genuine = false, seed = 8101) {
    ...
    Object.assign(o, { character: 1, kind: 'seller', itemId: 'clock-01', grade: 3, claimedGrade: 3, condition: 2, genuine, sleeper: false, apparent: 520, value: genuine ? 520 : 32, estimate: 520, asking: 280, price: 280, baseFloor: 220, floor: 220, paid: 0, sale: 0, attempts: 0, status: 'offered', negotiated: false, acceptedOffer: false, lastOffer: null, talkLeft: 3, patience: 6, mood: 0, relationDelta: 0, talked: [], physicalUsed: false, inspectionLeft: 2, inspected: [], inspectionResults: {}, suspicion: 0, leverage: 0, checks: [], clues: {}, lastCheck: null, verdict: '', feedback: '', notes: [], intent: genuine ? 'honest' : 'fraud', line: '이 시계에 얽힌 이야기는 값어치가 있지요. 280G면 어떻습니까?' });
    o.evidence = { physical: genuine ? .8 : .1, contradiction: !genuine, year: 2016, origin: '골목 경매', pressure: .8 };
  ...
  function buyerScene(fake = false) {
    ...
    Object.assign(o, { character: 0, kind: 'buyer', uid: 'practice-buyer', buyerQuotes: {}, talkLeft: 3, intent: 'honest', patience: 6, notes: [], history: [], talked: [], mood: 0, relationDelta: 0 });
  ...
  function ensureSellerTalk(...ids) {
    modify(s => { if (s.current.kind !== 'seller' || s.current.status !== 'offered') s.current = copy(sellerScene(false).current); for (const id of ids) if (!s.current.talked.includes(id)) { s.current.talkLeft = Math.max(s.current.talkLeft, 1); E.talk(s, id); } s.current.talkLeft = Math.max(s.current.talkLeft, 1); });
  }
  ...
  const steps = [
    ...
    { chapter: '말 속의 모순', title: '결과를 읽고 다음 손님 맞이하기', setup: () => { if (A.state.current.status === 'offered') modify(s => E.accept(s)); }, copy: '매입한 가격과 실제 가치를 비교하세요. 거짓말을 알아채도 얼마에 살지는 내 선택입니다. “보관하고 구매 손님 기다리기”를 눌러 넘어가 보세요.', goals: [actionGoal('거래 결과에서 다음 손님 맞이하기', 'next', undefined, '[data-action="next"]')], success: '다음 연습에서는 같은 시계의 진품을 가져온 손님을 만납니다.' },
    { chapter: '관계를 망치는 말', title: '말을 듣고, 조사할 곳을 고르세요', copy: '이번 연습의 시계는 진품입니다. 먼저 출처를 물은 뒤 돋보기로 원하는 지점 한 곳을 조사하세요. 조사할 곳을 알려 주는 정답 표시는 없습니다. 들은 말과 직접 본 흔적을 비교합니다.', setup: () => setState(sellerScene(true, 8102)), view: 'trade', goals: [actionGoal('손님에게 출처 묻기', 'talk', 'origin', '[data-talk="origin"]'), { label: '돋보기 도구 고르기', target: '#investigation-panel .investigation-tools', test: (_s, event) => event?.type === 'click' && event.element?.matches('[data-tool="lens"]') }, { label: '돋보기로 내가 고른 지점 조사', target: '#investigation-panel .investigation-targets', test: (_s, event) => eventMatches(event, 'investigate') && String(event.args[0]).startsWith('lens-') }], success: '관측한 흔적은 증거일 뿐 진위 판정이 아닙니다. 돋보기와 불빛 조사는 대화와 별도로 총 2회입니다.' },
    { chapter: '관계를 망치는 말', title: '불빛으로 다른 흔적 확인하기', copy: '조사 영역에서 불빛 도구를 고르고 각인·재료·마모 중 원하는 곳을 조사하세요. 남은 조사 기회를 어디에 쓸지는 내 판단입니다. 자동으로 정답을 알려 주지는 않아요.', goals: [{ label: '불빛 도구 고르기', target: '#investigation-panel .investigation-tools', test: (_s, event) => event?.type === 'click' && event.element?.matches('[data-tool="light"]') }, { label: '불빛으로 내가 고른 지점 조사', target: '#investigation-panel .investigation-targets', test: (_s, event) => eventMatches(event, 'investigate') && String(event.args[0]).startsWith('light-') }], success: '두 도구를 사용했어요. 같은 결과는 다시 읽을 수 있습니다. 추가 조사는 손님을 기다리게 해 인내심이 줄 수도 있어요.' },
    { chapter: '관계를 망치는 말', title: '무리한 압박의 대가를 체험하기', copy: '연습이니 일부러 “가짜라고 단정해 압박하기”를 고른 뒤, 1G를 제안해 보세요. 진품을 가품으로 몰거나 지나치게 낮은 가격을 부르면 관계가 나빠집니다.', setup: () => { ensureSellerTalk('origin', 'detail'); modify(s => { s.current.patience = 6; }); }, goals: [actionGoal('가짜라고 단정해 압박하기', 'talk', 'accuse', '[data-talk="accuse"]'), { label: '1G를 제안해 상대 반응 보기', target: '#offer-form', test: (_s, event) => eventMatches(event, 'offer', 1) }], success: '호감도와 인내심이 줄었습니다. 우호적인 손님은 흥정에 너그럽지만 계속 무리하면 떠납니다.' },
    { chapter: '구매 손님과 흥정', title: '이번에는 내가 판매하는 차례', copy: '보관함에 물건이 있으면 구매 손님도 찾아옵니다. 손님에게 보여 줄 물건 중 “좋아하는 품목” 표시가 있는 물건을 골라 보세요.', setup: () => setState(buyerScene(false)), view: 'trade', goals: [actionGoal('취향에 맞는 재고를 골라 보여 주기', 'chooseStock', 'practice-favorite', '[data-stock-choice="practice-favorite"]')], success: '같은 손님도 물건을 바꾸면 제안가가 달라집니다. 대화 기회와 인내심은 되돌아오지 않아요.' },
    ...
    { chapter: '내 가게를 키우기', title: '세 가지 가게 강화 직접 해 보기', copy: '연습 자금으로 감정사의 안목, 진열장 확장, 골목의 명성을 각각 한 번 강화하세요. 안목 4단계부터 대화 기회가 4회가 되지만 진위가 자동으로 드러나지는 않습니다.', setup: () => modify(s => { s.coins = Math.max(s.coins, 3000); s.upgrades = { appraisal: 0, shelves: 0, word: 0 }; }), view: 'shop', goals: [actionGoal('감정사의 안목 강화', 'upgrade', 'appraisal', '[data-action="upgrade"][data-key="appraisal"]'), actionGoal('진열장 확장', 'upgrade', 'shelves', '[data-action="upgrade"][data-key="shelves"]'), actionGoal('골목의 명성 강화', 'upgrade', 'word', '[data-action="upgrade"][data-key="word"]')], success: '판독 능력, 재고 공간, 판매와 흥정의 여지가 커졌습니다. 각 강화는 5단계까지 가능합니다.' },
  ...
  function finish() {
    menu.innerHTML = '<div class="onboarding-dialog-mark">✦</div><p class="title-kicker">READY TO OPEN</p><h2>이제 당신의 가게입니다</h2><p>말 속의 모순을 읽고, 내 가격을 정하고, 손님의 마음을 움직여 보세요.</p><p class="onboarding-safe-note">체험에서 쓴 돈과 거래는 실제 저장에 반영되지 않았습니다.</p><div class="onboarding-dialog-actions"><button class="pixel-menu-button new-game" data-tutorial="finish">시작 화면으로</button><button class="pixel-menu-button" data-tutorial="chapters">원하는 장면 다시 연습</button></div>';
    menu.showModal();
  }
